...
const { create, update, delete: deleteIndex, suffix } = require('./indices')
const { es } = require('./es')
...
describe('delete', () => {
  ...
  it(`should allow deleting an index and its alias`, async () => {
    ...
    const res = await deleteIndex(name)
    expect(res.name).toBe(name)
    expect(res.index).toMatch(new RegExp(`^${name}-\\d+-\\d+-\\d+t\\d+:\\d+:\\d+.\\d+z`))
    expect(res.ops).toHaveProperty('preChecks')
    expect(res.ops.preChecks).toEqual(expect.any(Array))
    ...
    expect(res.ops.preChecks).toEqual(expect.any(Array))
    expect(res.ops.deletions).toHaveProperty('length', 2)
    expect(res.ops.deletions[0]).toBeDefined()
    expect(res.ops.deletions[0]).toHaveProperty('aliasDeletion')
    expect(res.ops.deletions[0].aliasDeletion).toHaveProperty('acknowledged', true)
    expect(res.ops.deletions[1]).toBeDefined()
    expect(res.ops.deletions[1]).toHaveProperty('indexDeletion')
    expect(res.ops.deletions[1].indexDeletion).toHaveProperty('acknowledged', true)
  })
  ...
  it(`should forbid deleting an unknown index`, async () => {
    ...
    expect.assertions(3)
    try {
      await deleteIndex(`${name}-invalid`)
    } catch (error) {
      expect(error).toBeDefined()
