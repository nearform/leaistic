const { indexName, indexNameWithoutSuffix, indexTemplate } = require('./validation')
...
describe('indexTemplate', () => {
  it(`should allow a valid mapping`, () => {
    const validation = indexTemplate.validate({
      'index_patterns': ['*'],
      'order': 0,
      'settings': {
        'number_of_shards': 1
  ...
  it(`should allow undefined as it is not mandatory`, () => {
    const validation = indexTemplate.validate(undefined)
    expect(validation.error).toBeNull()
  })
})
