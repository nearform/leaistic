...
describe('rollbackAliasSwitch', () => {
  ...
  it(`should error when trying to remove an unknown index from alias`, async () => {
    const fakeIndex = suffix(uuid())
    expect.assertions(3)
    try {
      await rollbackAliasSwitch({name, sourceIndex, index: fakeIndex}, Error('"should error when trying to revert an unknown alias" test cause'), `"should error when trying to revert an unknown alias" test`)
    } catch (e) {
      expect(e).toBeDefined()
      expect(await aliasExists(name)).toBeTruthy()
